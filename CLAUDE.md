...
- **Password gate:** `plp.html` shows a password prompt iff `password.enc`
  ...
  180-day `plp_auth=ok` cookie. Verification is client-side via the vendored
  `assets/bcrypt.min.js` (bcryptjs 2.4.3; htpasswd's `$2y$` hashes verified
  compatible). It **fails open** (missing/malformed file or library → no gate)
  and is a courtesy gate only: the cookie holds no secret, the hash is public
  (offline brute-force of weak passwords is possible), and games.txt/CSVs stay
  directly fetchable. Real protection = Amplify access control (basic auth).
