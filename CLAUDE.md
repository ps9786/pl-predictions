...
- **Password protection is AWS Amplify access control** (App settings →
  Access control, HTTP Basic Auth enforced at the CDN edge), configured in the
  AWS Console — not in this repo. There is deliberately no client-side
  password gate: a prior attempt at one (bcrypt hash + JS prompt in `plp.html`)
  was removed in favour of Amplify's real, server-enforced auth. Don't re-add
  a client-side gate; if the site needs protecting, that's an Amplify Console
  setting.
