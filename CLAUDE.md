...
- **Password gate:** `plp.html` shows a password prompt iff `password.enc`
  (bcrypt hash, generated by `tools/make_password.sh`) exists; success sets a
  180-day `plp_auth=ok` cookie so the prompt doesn't reappear on later visits.
  Verification is client-side via the vendored `assets/bcrypt.min.js`
  (bcryptjs 2.4.3; htpasswd's `$2y$` hashes verified compatible). It **fails
  open** (missing/malformed file or library → no gate). This is deliberately
  a **courtesy gate, not real security** — it's meant to stop casual/script-
  kiddie access, not a determined visitor: the cookie holds no secret, the
  hash is public (offline brute-force of weak passwords is possible), and
  games.txt/CSVs stay directly fetchable regardless. Amplify access control
  (basic auth, set in the AWS Console) is the option for actual protection if
  ever needed, but is not required for this gate to make sense on its own.
...
League scoring (`tools/calculate_scores_table.py`, run daily by
`tools/daily_update.sh`): **5 points** exact score no other player predicted,
**3 points** exact score shared with another player, **1 point** correct
result, 0 otherwise. Inputs: `selections.csv` (root; `Match Date,Match No,
FIXTURE,<players…>`, fixtures as `Home v Away`) + `results_2026_27.csv`
(from `tools/fetch_pl_results.sh`). Output: `league_table.csv`. The `Match No`
